...
async function fetchWeather() {
    ...
    if (!icao) {
        output.textContent = "Please enter an ICAO code.";
        return;
    }

    // Using AVWX API JSON endpoint (free, no key needed for basic METAR)
    const url = `https://avwx.rest/api/metar/${icao}?format=json`;

    try {
        const res = await fetch(url, {
            headers: {
                "Authorization": "YOUR_API_KEY_HERE" // Optional: some endpoints need API key
            }
        });
        if (!res.ok) throw new Error("Weather not found");
        const data = await res.json();

        // Display key METAR info
        output.textContent = 
`Station: ${data.station}
Time: ${data.time.dt || data.time}
Wind: ${data.wind_dir || data.wind_direction} at ${data.wind_speed || data.wind_speed_kts} kts
Visibility: ${data.visibility || "N/A"}
Temperature: ${data.temperature || "N/A"} °C
Condition: ${data.wx || "N/A"}
Raw METAR: ${data.raw || "N/A"}`;
    } catch (err) {
